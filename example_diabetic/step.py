...
data = csv.reader(open('step.csv', 'rb'))
replay = []
for row in data:
	replay.append(row)
...
# read discretization from CSV file
apm_option(server,app,'nlc.spc_chart',0)
...
apm_option(server,app,'g.splo',80)
...
apm_option(server,app,'u.status',0)
apm_option(server,app,'u.fstatus',1)
